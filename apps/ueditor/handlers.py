...
class UploadHandler(BaseHandler):
    ...
    def post(self):
        ...
        if action == u4Ts.config["imageActionName"]:
            for keys in self.request.files:
                for fileobj in self.request.files[keys]:
                    data = yield self.save_file(
                        base_dir=u4Ts.config["imagePathFormat"], fileobj=fileobj
                    )
            ...
            fileobj = {
                "filename": "scrawl.png",
                "body": base64.decodebytes(
                    self.get_argument(u4Ts.config["scrawlFieldName"]).encode("utf-8")
                ),
            }
            data = yield self.save_file(
            ...
            for keys in self.request.files:
                for fileobj in self.request.files[keys]:
                    data = yield self.save_file(
                        base_dir=u4Ts.config["snapscreenPathFormat"], fileobj=fileobj
                    )
            ...
            for keys in self.request.files:
                for fileobj in self.request.files[keys]:
                    data = yield self.save_file(
                        base_dir=u4Ts.config["videoPathFormat"], fileobj=fileobj
                    )
